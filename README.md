...
# 分享给他人（需要 cloudflared）
cloudflared tunnel --url http://localhost:8090
# → 获得 https://xxx.trycloudflare.com 公开链接
```

打开页面 → 填岗位名 → 手机扫码登录 BOSS → 实时看进度 → 查看分析报告。详细介绍见 [doc/requirements-v2.md](doc/requirements-v2.md)。
...
pip install cloakbrowser playwright jieba pyyaml requests fastapi "uvicorn[standard]"
```
...
```
浏览器表单  →  web_app.py (FastAPI)  →  tools/scraper.py   抓取 → data/*.json
                                     →  tools/analyzer.py   分析 → reports/*.html
                                     →  tools/publisher.py  推送 → 飞书卡片
```

启动 `python web_app.py` 即可，访问 `http://localhost:8090`。
...
| 模块 | 职责 | 独立运行 |
|------|------|----------|
| [web_app.py](web_app.py) | Web 服务入口（FastAPI） | `python web_app.py` |
...
└── boss-profile/                           # 浏览器持久化 Profile（Cookie 等）
```
